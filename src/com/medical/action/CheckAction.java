...
public class CheckAction extends ActionSupport {
	...
	public String checkQuery() {
		...
		cds = baseinfoService.findAllMemberInfo("checkQuery.action", cur_page,
				sql);
	...
	public String checkSsn() {
		...
		cds = baseinfoService.findAllMemberInfo("checkSsn.action", cur_page,
				sql);
		setToolsmenu(baseinfoService.getPager().getToolsmenu());
		this.setOrgs(this.searchService.getOrganList(user.getOrganizationId()));
		return SUCCESS;
	...
	@SuppressWarnings("rawtypes")
	public String installSsnInit() throws UnsupportedEncodingException,
			DocumentException {
		...
		try {
			...
			String xml = iService1.getMedicareInfoSingle(paperid.toUpperCase(),
					name);
			...
			if ("1".equals(resultFlag)) {
				List list = document
						.selectNodes("//GetMedicareInfoSingle/NewDataSet/jljzj");
				...
				while (iter.hasNext()) {
					...
					String cbzt = ele.element("医保参保状态").getText();
	...
	public String installSsn() {
		...
		try {
			...
			String xml1 = iService1.setAssistStatusSingle(icid.toUpperCase(),
					checkDTO.getMembername(), checkDTO.getFamilyno(), status,
					dateString);
			...
			this.baseinfoService
					.saveccc(xml1, datatype, checkDTO.getMemberId());
